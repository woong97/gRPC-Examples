...
func uploadImage(laptopClient pb.LaptopServiceClient, laptopID string, imagePath string) {
	...
	err = stream.Send(req)
	if err != nil {
		log.Fatal("cannot send iamge info: ", err, stream.RecvMsg(err))
	}
	...
	if err != nil {
		log.Fatal("cannot receive response: ", err, stream.RecvMsg(err))
	}
...
func rateLaptop(laptopClient pb.LaptopServiceClient, laptopIDs []string, scores []float64) error {
	...
	if err != nil {
		return fmt.Errorf("cannor rate laptop: %v", err)
	}
